...
export default function AccessRemovalPendingPage() {
  ...
  if (!session || session.role !== "Admin") {
    return (
      <OneFlowShell title="Access removal pending">
  ...
  return (
    <OneFlowShell
      title="Access removal pending"
      subtitle="Security-critical offboarding tasks not yet completed"
    >
              ...
              <tr key={task.id} className="border-t border-flow-line">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1">
                    {task.executionMode && (
                      <StatusChip status={task.executionMode} />
                    )}
            ...
            {rows.length === 0 && (
              <tr>
                <td className="px-4 py-8 text-slate-400" colSpan={6}>
                  No pending access removal tasks.
                </td>
              </tr>
            )}
          </tbody>
