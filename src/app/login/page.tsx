...
export default function LoginPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-950 font-flow text-slate-900">
      ...
      <div className="relative mx-auto grid min-h-screen max-w-[1160px] gap-6 px-4 py-5 sm:px-7 lg:grid-cols-[.62fr_1.38fr] lg:items-center lg:px-8 xl:px-10">
        ...
        <section className="rounded-[20px] border border-white/35 bg-gradient-to-br from-white/[.96] via-white/[.93] to-slate-100/[.92] p-4 shadow-2xl shadow-slate-950/35 backdrop-blur-xl sm:p-6 lg:max-h-[calc(100vh-2.5rem)]">
          <header className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200/80 pb-4">
            <div><p className="text-[13px] font-semibold uppercase tracking-[0.17em] text-flow-accent">OneFlow workspace</p><h2 className="mt-1 text-[26px] font-semibold tracking-tight text-slate-900">Welcome back</h2><p className="mt-1 text-[15px] text-slate-500">Choose a role to explore OneFlow.</p></div>
          ...
          <div className="rounded-2xl border border-sky-100 bg-gradient-to-r from-sky-50/85 via-white to-violet-50/80 p-3.5 shadow-sm">
            <div className="flex items-center justify-between gap-3"><div><h3 className="text-[13px] font-semibold uppercase tracking-[0.15em] text-slate-600">Explore Employee Journeys</h3><p className="mt-1 text-[13px] text-slate-500">See OneFlow from the employee perspective.</p></div><Sparkles className="h-4 w-4 text-violet-500" /></div>
            <div className="mt-3 grid gap-2.5 sm:grid-cols-2">{journeys.map((user) => { const detail = journeyCopy(user); return <button key={user.id} type="button" onClick={() => selectAccount(user)} className={`group rounded-xl border ${detail.accent} p-3 text-left shadow-sm transition duration-150 hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-flow-accent focus:ring-offset-2`}><div className="flex gap-2.5"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full border text-[11px] font-bold ${detail.avatar}`}>{initials(user)}</span><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="text-[15px] font-semibold leading-5 text-slate-800 sm:truncate">{user.name}</p><ArrowRight className="mt-0.5 h-4 w-4 shrink-0 text-slate-500 transition group-hover:translate-x-0.5 group-hover:text-flow-accent" /></div><p className="mt-0.5 truncate text-[13px] text-slate-500">{user.email}</p><div className="mt-2 flex flex-wrap gap-1"><span className={`rounded-md px-1.5 py-0.5 text-[11px] font-semibold ${detail.badge}`}>{detail.status}</span><span className="rounded-md bg-white/80 px-1.5 py-0.5 text-[11px] font-medium text-slate-600">{roleLabel(user.role)}</span></div></div></div><p className="mt-2 text-[13px] leading-5 text-slate-600">{detail.copy}</p><span className="mt-2 inline-flex items-center text-[13px] font-semibold text-flow-accent">Explore Journey <ArrowRight className="ml-1 h-3.5 w-3.5 transition group-hover:translate-x-0.5" /></span></button>; })}</div>
          </div>

          <div className="mt-3.5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-[13px] font-semibold uppercase tracking-[0.15em] text-slate-600">Demo Accounts</h3><p className="mt-0.5 text-[13px] text-slate-500">Choose a workspace role to continue.</p></div><div className="flex flex-wrap gap-1.5">{FILTERS.map((item) => <button key={item.label} type="button" onClick={() => setFilter(item.label)} className={`rounded-full px-2.5 py-1.5 text-[11px] font-semibold transition focus:outline-none focus:ring-2 focus:ring-flow-accent focus:ring-offset-1 ${filter === item.label ? "bg-slate-800 text-white shadow-sm" : "border border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50"}`}>{item.label}</button>)}</div></div>

          <div className="mt-2.5 max-h-[31vh] space-y-2.5 overflow-y-auto pr-1 sm:max-h-[33vh]">{filter === "Employee Journeys" ? <p className="rounded-xl border border-dashed border-sky-200 bg-sky-50/50 px-4 py-5 text-center text-[13px] text-slate-600">Employee journey accounts are highlighted above.</p> : groupedUsers.map((group) => <section key={group.label}><h3 className="mb-1.5 text-xs font-semibold uppercase tracking-[0.15em] text-slate-500">{group.label}</h3><div className="grid gap-2 sm:grid-cols-2">{group.users.map((user) => <button key={user.id} type="button" onClick={() => selectAccount(user)} className={`group flex min-h-[80px] items-center gap-2.5 rounded-xl border border-slate-200 border-l-[3px] ${group.accent} bg-white px-3 py-2.5 text-left shadow-sm transition duration-150 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-flow-accent focus:ring-offset-2`}><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border text-[10px] font-bold ${group.avatar}`}>{initials(user)}</span><span className="min-w-0 flex-1"><span className="block truncate text-[15px] font-semibold text-slate-800">{user.name}</span><span className="mt-0.5 block truncate text-[13px] text-slate-500">{user.email}</span><span className={`mt-1.5 inline-flex rounded-md px-1.5 py-0.5 text-[11px] font-semibold ${group.badge}`}>{roleLabel(user.role)}</span></span><ArrowRight className="h-4 w-4 shrink-0 text-slate-400 transition group-hover:translate-x-0.5 group-hover:text-flow-accent" /></button>)}</div></section>)}</div>
